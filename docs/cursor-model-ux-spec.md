...
- Exact `@cursor/sdk@1.0.31` is a package dependency of this extension; users should not need a global SDK install. Pi 0.84.0 remains the minimum supported; Pi 0.84.3 is the current validation baseline, while optional published Pi core peer dependencies use `"*"` ranges per current Pi package guidance.
