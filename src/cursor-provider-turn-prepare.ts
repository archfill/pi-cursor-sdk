...
function buildCursorCloudPromptContext(
  context: Context,
  handoff: "fresh" | "bootstrap" | "never",
): Context {
...
const CLOUD_SEND_PLAN: CursorSessionSendPlan = {
  mode: "bootstrap",
  resetAgent: false,
  reason: "initial",
};
...
function buildCloudCursorProviderTurnLifecycle(
  agent: SDKAgent,
): CursorProviderTurnLifecycle {
  return {
    trackRunCompletion: () => {},
...
function buildLocalCursorProviderTurnLifecycle(
  ...
  return {
    ...
    commitSend: (context, bootstrapped) =>
      lease.commitSend(context, bootstrapped),
...
async function prepareCursorCloudProviderTurn(
  ...
  try {
    ...
    if (
      getPendingCursorLiveRun(context) ||
      getActiveCursorLiveRunForCurrentScope()
    ) {
      throw new Error(
        "Cursor cloud runtime cannot start while a local Cursor live run is pending; finish or abort the local run, then retry.",
      );
    }
    ...
    const prompt = buildCursorPrompt(
      buildCursorCloudPromptContext(
        context,
        resolvedConfig.cloud.contextHandoff.value,
      ),
      promptOptions,
    );
    ...
    if (
      ...
      throw createCursorCloudLifecyclePersistenceError(
        agent.agentId,
        "intent",
        undefined,
        resolvedApiKey,
...
async function prepareCursorLocalProviderTurn(
  ...
  try {
    ...
    const buildPromptOptions = (
      ...
      const includeToolManifest =
        resolveCursorToolManifestEnabled() &&
        (plan.mode === "bootstrap" || plan.reason === "post_compaction");
      if (!includeToolManifest) return promptOptions;
      return {
        ...promptOptions,
    ...
    const forcePostCompactionBootstrap =
      consumePostCompactionBootstrapRequired(sessionAgentScopeKey);
    let sendPlan = planCursorSessionSend(sessionAgentLease.sendState, context, {
      forcePostCompactionBootstrap,
    });
    if (
      sessionAgentLease.created &&
      sessionAgentLease.resumed &&
      sendPlan.mode === "incremental" &&
      sendPlan.reason !== "post_compaction"
    ) {
      sendPlan = {
        mode: "bootstrap",
        resetAgent: false,
    ...
    return {
      ...
      meta: {
        ...
        agentMode,
        modelSelection: selection,
        ...(sessionAgentLease.resumeNotice
          ? { resumeNotice: sessionAgentLease.resumeNotice }
          : {}),
      },
      contextWindowAgentId: agent.agentId,
      textDeltas,
...
export async function prepareCursorProviderTurn(
  ...
  const fastEnabled =
    resolvedConfig.runtime.value === "cloud"
      ? undefined
      : getEffectiveFastForModelId(model.id);
