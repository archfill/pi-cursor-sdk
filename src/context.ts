...
export interface CursorPromptOptions {
  ...
  toolManifest?: string;
  /** Re-inject tool boundary (and toolManifest when set) on incremental sends, e.g. after compaction. */
  includeToolBoundary?: boolean;
...
export function getCursorToolTailGuardText(
  options: Pick<CursorPromptOptions, "agentMode"> & {
    includePlanModeGuidance?: boolean;
    includePiBridgeGuidance?: boolean;
  } = {},
): string {
  return [
...
function getCursorToolBoundaryText(
  options: Pick<
    CursorPromptOptions,
    "agentMode" | "includePiAskQuestionGuidance"
  > & { hasToolManifest?: boolean; includePiBridgeGuidance?: boolean } = {},
): string {
  const includePiBridgeGuidance = options.includePiBridgeGuidance !== false;
  const includePiAskQuestionGuidance =
    includePiBridgeGuidance && options.includePiAskQuestionGuidance !== false;
  const lines = [
...
function getCursorBootstrapTailSections(
  options: Pick<
    CursorPromptOptions,
    "agentMode" | "includePiBridgeGuidance"
  > = {},
): string[] {
  return [
...
function applyPromptBudget(
  ...
  const includedMessageIndexes = new Set(
    requiredMessageSections.map((section) => section.index),
  );
  ...
  for (let index = messageSections.length - 1; index >= 0; index -= 1) {
    ...
    omittedMessageCount += messageSections
      .slice(0, index + 1)
      .filter(
        (candidate) => !includedMessageIndexes.has(candidate.index),
      ).length;
    break;
  }
...
export function estimateCursorPromptTokens(
  ...
  options: Pick<
    CursorPromptOptions,
    "charsPerToken" | "imageTokenEstimate"
  > = {},
): number {
  return (
    estimateCursorTextTokens(prompt.text, options) +
    prompt.images.length *
      (options.imageTokenEstimate ?? CURSOR_IMAGE_TOKEN_ESTIMATE)
  );
}
...
export function estimateCursorContextTokens(
  ...
  return estimateCursorPromptTokens(
...
function serializeMessageForFingerprint(
  message: Message,
  index: number,
): string {
  switch (message.role) {
    case "user": {
      ...
      return hashCursorContextValue(
        `user:${message.timestamp ?? index}:${text}`,
      );
    }
    case "assistant":
      return hashCursorContextValue(
  ...
  switch (role) {
    ...
    case "bashExecution": {
      ...
      if (entry.excludeFromContext) {
        return hashCursorContextValue(
          `bashExecution:excluded:${entry.timestamp ?? index}`,
        );
      }
      return hashCursorContextValue(
...
export function shouldBootstrapCursorContext(
  ...
  const current = parseCursorContextFingerprint(
    computeCursorContextFingerprint(context),
  );
  ...
  if (current.messageHashes.length > previous.messageHashes.length) {
    for (
      let index = previous.messageHashes.length;
...
export function buildCursorIncrementalPrompt(
  ...
  // Incremental sends omit Pi system instructions and the full transcript; the session agent retains bootstrap context.
  // Post-compaction sends may re-inject only the tool boundary and manifest without replaying tool-call transcript lines.
  ...
  if (options.includeToolBoundary) {
    sectionsBeforeMessages.push(
      getCursorToolBoundaryText({
    ...
    if (options.toolManifest) sectionsBeforeMessages.push(options.toolManifest);
  }
  sectionsBeforeMessages.push(
    ...
    options.maxInputTokens === undefined
      ...
      : {
          ...
          maxInputTokens: Math.max(
            1,
            options.maxInputTokens - imageTokenReserve,
          ),
        };
...
export function buildCursorPrompt(
  ...
  const messageSections = messages
    ...
    .filter(
      (section): section is { index: number; text: string } =>
        section !== undefined,
    );
