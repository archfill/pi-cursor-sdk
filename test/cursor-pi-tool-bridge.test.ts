...
describe("cursor pi tool bridge flags and snapshots", () => {
	...
	it("builds bounded provider-facing bridge tool call IDs", () => {
		...
		expect(isCursorPiBridgeToolCallId("cursor-pi-bridge-run-c2417032-9686-40f2-bd3c-a763fbaa7693-tool-1")).toBe(true);
		...
		expect(() => buildCursorPiBridgeToolCallId(runUuid, 0)).toThrow(
...
describe("cursor pi tool bridge loopback MCP lifecycle", () => {
	...
	it("uses endpoint-independent request IDs so historical tool results cannot resolve a new run", async () => {
		...
		try {
			...
			expect(request.piToolCallId).toMatch(/^cursor-pi-bridge-[0-9a-f]{32}-t\d+$/i);
			expect(request.piToolCallId).toHaveLength(52);
	...
	it("emits scrubbed request diagnostics for queue, resolution, cancellation, and rejection", async () => {
		...
		try {
			...
			expect(resolvedRequest.piToolCallId).toMatch(/^cursor-pi-bridge-[0-9a-f]{32}-t\d+$/i);
			expect(resolvedRequest.piToolCallId).toHaveLength(52);
